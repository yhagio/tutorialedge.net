...
* **Authorization** - The most commonly used practice. Once a user logs in to your application, or authenticates in some manner, every request that is then sent from the client on behalf of the user will contain the JWT.  
...
So, we are going to be using the code from one of my other articles, [Creating a simple REST API in Go](/golang/creating-restful-api-with-golang/), to get us started. This will feature a really simple `Hello World` style endpoint and it will run on port 8081.
...
If you go down this route, you'll need to give them a copy of the string you used to sign your JWT. This obviously isn't ideal and if you give it out to a malicious third-party then you could see attacks taking place.
